...
import { Bold, Italic, Link as LinkIcon, List, ListOrdered, Maximize2, Minimize2, Plus, Type, X } from 'lucide-react'
...
export function DocumentTodo() {
  ...
  const updateTodoContent = (id: string, content: string, parentPath: string[] = []) => {
    setTodos(current => {
      const updated = [...current]
      let target = updated
  ...
  const renderTodoPanel = (todo: Todo | null, todos: Todo[], path: string[] = [], isRoot = false) => {
    const isActive = path.length === activePath.length && 
  ...
  const getPanels = () => {
    const panels: JSX.Element[] = [renderTodoPanel(null, todos, [], true)]
      ...
      if (todo) {
        panels.push(
          renderTodoPanel(
            todo,
            todo.children,
            activePath.slice(0, panels.length)
          )
        )
